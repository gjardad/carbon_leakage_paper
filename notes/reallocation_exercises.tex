...
\section{Six-channel pairwise decomposition}
\label{sec:pairwise}

\textbf{Script:} \texttt{analysis/phase0\_pairwise\_decomposition.R}.

Runs the same Grossman--Krueger accounting as \S\ref{sec:sct} but for arbitrary $(t_{\text{base}}, t_{\text{end}})$ pairs rather than fixed-base 2005. Adds explicit entry and exit channels.

\subsection{Variables}
Same primitives as \S\ref{sec:sct}: $e_{i,t}$, $q_{i,t}$, $Y_t$. For a given pair $(t_0, t_1)$:
\begin{itemize}
  \item Survivors $S$: firms with $e_{i,t} > 0$ for both $t = t_0$ and $t = t_1$. Joined on firm ID (\texttt{vat}); a firm that switches NACE 2d between years is still a survivor, and its $t_0$ sector is used for the between/within split.
  \item Entrants $N$: firms with $e_{i,t_1} > 0$ and $e_{i,t_0} = 0$ (or no row in $t_0$).
  \item Exiters $X$: firms with $e_{i,t_0} > 0$ and $e_{i,t_1} = 0$ (or no row in $t_1$).
\end{itemize}
Within-survivor aggregates: $Y_{t,S} = \sum_{i \in S} q_{i,t}$, $\theta_{i,t}^S = q_{i,t} / Y_{t,S}$, $z_{i,t} = e_{i,t}/q_{i,t}$.
...
\end{equation}
with the survivor portion computed by the standard counterfactual construction:
\begin{align*}
...
\end{align*}
The six channels sum to $E_{t_1} - E_{t_0}$ exactly. All channels reported in pp of $E_{t_0}$.

\subsection{Identification assumption}
Mechanical accounting. Same caveat as \S\ref{sec:sct}: no causal claim.

\subsection{Entrant and exiter intensities}

Beyond the six-channel accounting, the emissions-weighted mean intensity of entrants at $t_{\text{end}}$ and of exiters at $t_{\text{base}}$ is informative:
\begin{align*}
  \bar z^E &= \frac{\sum_{i \in N} e_{i,t_1}}{\sum_{i \in N} q_{i,t_1}}, &
  \bar z^X &= \frac{\sum_{i \in X} e_{i,t_0}}{\sum_{i \in X} q_{i,t_0}}.
\end{align*}
These are reported in tCO\textsubscript{2} per million EUR of real revenue in the table below.

\subsection{Triple-balanced panel}

For a strict ``same firms'' comparison across $2005/2012/2020$, restrict to the 101 firms with positive emissions and revenue in all three years. On this panel entry and exit are zero by construction, and technique can be compared directly across the $2005 \to 2012$ and $2012 \to 2020$ windows on the same firm set.
...
\paragraph{Takeaway --- the two phases.} The fall is not uniform across time. Between 2005 and 2012 emissions fell $16\%$, and the within-firm abatement channel alone would have delivered a $50\%$ fall: technique $= -49.6$, while scale ($+10.8$), between-sector ($+7.4$), and within-sector ($+10.5$) were all pushing emissions up. Between 2012 and 2022 the picture inverts. Emissions fell only $13\%$, the technique channel essentially disappears (technique $= -0.9$), and between-sector reallocation carries almost the entire decline ($-29.5$). The conclusion is that the abatement story is almost entirely a Phase I/II phenomenon; from 2012 onward the emissions decline comes from the Belgian manufacturing mix shifting away from high-emission sectors, not from surviving ETS firms getting cleaner. The triple-balanced rows tell the same story on an even tighter sample: technique $-54$ over 2005--2012 and $+10$ over 2012--2020 among the same 101 firms.

\subsection{What this pins down that Table 1 could not}

\textbf{(1) Did Phase I/II abatement persist?} No. On the triple-balanced 101-firm panel --- firms present in all three of 2005, 2012, and 2020 --- technique is $-53.7$ pp over $2005 \to 2012$ but $+9.5$ pp over $2012 \to 2020$. Among literally the same firms, a meaningful fraction of the earlier intensity reduction reversed from 2012 onward. The 2005 $\to$ 2020 full-window technique is $-23.1$ on this panel: essentially the net of $-53.7$ and $+9.5$, with Laspeyres/Paasche weighting differences explaining the slight gap. The reversal was already visible in the unrestricted $2012 \to 2020$ pair ($+4.2$, 126 firms) and $2007 \to 2020$ pair ($+2.6$, 109 firms); the triple-balanced panel makes it sharper because the sample is held fixed.

\textbf{(2) What is actually driving the post-2012 decline?} Between-sector reallocation. On the triple-balanced panel, between is $-16.2$ over $2012 \to 2020$ while technique is $+9.5$. The two components net to a total of only $-3.3$. The full 2005 $\to$ 2022 headline of $\sim -46\%$ is therefore a combination of two distinct phases: within-firm abatement concentrated in 2005--2012, and between-sector reallocation accelerating after 2012.

\textbf{(3) Are exiters dirtier than entrants? No --- the opposite.} $\bar z^E > \bar z^X$ in every unrestricted pair, with ratios $1.4$ to $2.4$. ``Exit = dirty firms dying under ETS pressure, entry = cleaner new firms'' is the wrong mental model for this data. Three plausible reasons: (i) ``exit'' is mostly Annual Accounts missingness rather than literal closure (EUTL installation roster is near-stable at 281 firms); (ii) entrants include installations in their capacity ramp-up phase, with elevated intensity; (iii) firms whose emissions crossed the zero threshold between the two years. Despite the entry-is-dirtier pattern, net emissions from entry minus exit is small ($|\text{Entry} - \text{Exit}| \leq 4.4$ pp) because exiters tend to be larger in revenue terms.

\subsection{EUTL reclassifications and the 2022 rows}

Diagnosed 2026-04-23. Inspection of per-firm emissions and free-allocation trajectories in the sectors whose post-2020 aggregate EUTL emissions diverge from Statbel aggregates (NACE 20, 24) reveals the following pattern. In each of NACE 20 and NACE 24, one large firm (plus a second smaller firm in NACE 20) shows:
\begin{itemize}
  \item EUTL verified emissions drop to $\sim 0$ in 2021, sustained in 2022;
  \item EUTL free allocation drops to $\sim 0$ or near-zero in 2021;
  \item Annual Accounts revenue continues or grows (in the NACE 24 case, revenue roughly doubles from 2020 to 2022).
\end{itemize}
This pattern is inconsistent with real abatement or firm closure and consistent with installation-level reassignment under Phase IV of the EU ETS (operator code change, EUTL registry event). NACE 21 does not show this pattern: its 2021 free-allocation cut is the intended Phase IV benchmark reset, and its emissions continue near trend.

The \emph{Excluded} rows in the table drop these three firms from every year of the sample (not just 2021+, which would merely relabel their 2005 emissions as ``Exit''). Effect on the 2005 $\to$ 2022 accounting:
\begin{itemize}
  \item Total $-37.3 \to -24.9$: about $-12$ pp of the unrestricted total was artefact.
  \item Technique $-25.6 \to -11.6$: the bulk of the cleanup --- these firms' reclassification-driven emissions drop was being counted as within-firm abatement.
  \item Between-sector $-31.7 \to -35.1$: slightly \emph{more} negative on the clean panel, i.e.\ between-sector reallocation is the genuine driver of the 2005 $\to$ 2022 decline, not the contaminated technique term.
\end{itemize}
The cleaned 2012 $\to$ 2022 row (Total $-12.6$, Technique $-0.9$, Between $-29.5$) is the sharpest statement of the paper's main story: post-2012, real within-firm abatement is essentially zero; the emissions decline is entirely between-sector reallocation. Pairs ending in 2020 or earlier are not affected by this correction and do not need to be re-run.

\subsection{Residual caveats}

\begin{itemize}
  \item Technique in the triple-balanced rows does not decompose exactly additively ($-53.7 + 9.5 \neq -23.1$). That is expected: the counterfactual weights $\theta_{i,t_1}$ differ across pairs (Paasche-style). The discrepancy ($-2.1$ pp out of $-23.1$) is small.
  \item ``Between'' uses only survivor aggregates, unlike the fixed-base Table 1 (which uses all firms at the sector level); numbers are not directly comparable across tables for overlapping years.
  \item The triple-balanced panel is a selected sample --- firms with continuous emissions and Annual Accounts coverage across 15 years are larger and more established than the broader population. The sign of the $2012 \to 2020$ reversal matches the 126-firm pair-wise estimate, so selection is not explaining the direction.
\end{itemize}
